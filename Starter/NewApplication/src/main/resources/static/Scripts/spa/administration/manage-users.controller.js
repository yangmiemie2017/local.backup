(function () {
    ...
    define(['app','services/webapi'], function (app) {
        ...
        function manageUsersCtrl(webapi, $state, $timeout, logger, $stateParams, $uibModal, $scope,Upload) {
            ...
            vm.uploadFile = function (file, isInvalid) {
                if (isInvalid) {
                    return;
                }
                vm.parameter="Datetime: "+new Date().Format("yyyy-MM-dd-hhmmss");
                var paras = {
                    "parameter": vm.parameter
                };

                var fileUpload = webapi.upload(file, paras);                

                fileUpload.then(function (response) {
                    $timeout(function () {
                        logger.success(response.data.data);

                        file.progress = 0;// reset progress bar
                        // reload
                        $timeout(function () {
                            $state.go('.', {}, { reload: true });
                        }, 100);
